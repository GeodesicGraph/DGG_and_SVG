...
public:
	...
	inline void decrement( int nIndex ) {
		...
		assert( m_vData[nIndex].nRefCount >= 0 );
	...
	inline int insert( const Type & t ) {
		...
		if ( m_nFirstFree == INVALID_REFCOUNT ) {
			...
			assert(nFree >= 0 && nFree < (int)m_vData.size());
			...
			if ( nNextFree < 0 )
				...
				assert( nNextFree == INVALID_REFCOUNT );
	...
	inline void remove( int nIndex ) {		// force remove
		assert( (unsigned int)nIndex < m_vData.size() );
	...
	inline Type & operator[]( int nIndex ) {
		assert( nIndex < (int)m_vData.size() && m_vData[nIndex].nRefCount > 0 && m_vData[nIndex].nRefCount != INVALID_REFCOUNT  );
...
public:
	...
	public:
		...
		inline index_iterator( index_iterator & copy ) {
	...
	protected:
		...
		inline index_iterator( int nIndex, RefEntry * pCurrent, RefEntry * pLast )
		{
			m_nIndex = nIndex;
			m_pCurrent = pCurrent;
			m_pLast = pLast;
	...
	inline index_iterator begin_indexes() {
		if ( m_vData.empty() )
			return end_indexes();
		else
			return index_iterator( (int)0, &m_vData.front(), &m_vData.back() + 1 );
	}
	inline index_iterator end_indexes() {
		if ( m_vData.size() == 0 ) 
