	...
	if (mode == GL_SELECT){
		...
		for (int i = 0; i < GetNumOfFaces(); ++i)
		{
			...
			for (int j = 0; j < 3; ++j)
			{
				...
				CPoint3D pt = Vert(Face(i)[j]) - normal * RateOfNormalShift;
...
void CBaseModel::Render(GLenum mode, const vector<float>* distance_field) const
{
	...
	if (mode == GL_SELECT){
		...
		if (distance_field == NULL) {
			for (int i = 0; i < GetNumOfFaces(); ++i)
			{
				for (int j = 0; j < 3; ++j)
				{
					CPoint3D pt = Vert(Face(i)[j]) + Normal(Face(i)[j]) * RateOfNormalShift * -1.0;
					const CPoint3D &normal = Normal(Face(i)[j]);
					//glTexCoord2d( distance_field[Face(i)[j]], distance_field[Face(i)[j]] );
					glNormal3f((float)normal.x, (float)normal.y, (float)normal.z);
					glVertex3f((float)pt.x, (float)pt.y, (float)pt.z);
